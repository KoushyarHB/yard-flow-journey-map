# Field: Gate Confirm — implementation prompt
...
## Goal
Field: Gate Confirm screen in the **Loading Pipeline** cluster. Used by Depot Tallyman, Control Office, Warehouse Guard, Vessel Tallyman.

## Route & placement
- Route: `(mobile handheld)`
- Sidebar: Yard Flow (L1 rail) → Loading & Exit (L2 cluster) → route cluster → Field: Gate Confirm (L4)
- Breadcrumb: Yard Flow / Loading / Field: Gate Confirm
...
- Endpoints:
  ...
  |--------|------|---------|-------------|--------------|
| `GET` | `/gate-exits/lookup` | Field: Gate Confirm action | — | — |
| `POST` | `/gate-exits/{lookup}/confirm` | Field: Gate Confirm action | — | — |
| `POST` | `/gate-exits/{lookup}/reject` | Field: Gate Confirm action | — | — |
- Auth: mutations require `actor` field. Permissions: gateexit.final.confirm.


## Data model (frontend types to add)
- `src/lib/types/yard-flow/response/gate-mobile/get-gate-mobile.dto.ts`
- `src/lib/types/yard-flow/request/gate-mobile/create-gate-mobile-request.dto.ts`
...
- Validation: Zod schema in `src/lib/schema/yard-flow/gate-mobileSchema.ts`.

## Files to create
- `src/app/[locale]/yard-flow/...` — thin route wrapper
- `src/components/pages/yard-flow/loading-pipeline/gate-mobile/`
- `src/services/yard-flow/gatecontrolService.ts`
- `src/hooks/yard-flow/useField:GateConfirmMutations.ts`
