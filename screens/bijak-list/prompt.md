...
## Goal
Bijaks screen in the **Loading Pipeline** cluster. Used by Depot Tallyman, Control Office, Warehouse Guard, Vessel Tallyman.

## Route & placement
- Route: `/yard-flow/loading/bijak`
- Sidebar: Yard Flow (L1 rail) → Loading & Exit (L2 cluster) → route cluster → Bijaks (L4)
- Breadcrumb: Yard Flow / Loading / Bijaks
...
- Endpoints:
  ...
  |--------|------|---------|-------------|--------------|
| `GET` | `/bijaks` | Bijaks action | — | — |
| `POST` | `/bijaks` | Bijaks action | — | — |
- Auth: mutations require `actor` field. Permissions: bijak.issue.


## Data model (frontend types to add)
- `src/lib/types/yard-flow/response/bijak-list/get-bijak-list.dto.ts`
- `src/lib/types/yard-flow/request/bijak-list/create-bijak-list-request.dto.ts`
- Enums: `src/lib/enums/yard-flow/bijak-status.enum.ts` — values: Draft, Issued, Printed, GateExitConfirmed, Closed
...
- **Payment** (`payment`) — filter: text, status badge

- Toolbar actions mapped to endpoints listed above.
- Status badges use semantic tones (green=confirmed, amber=draft, red=rejected, blue=in-progress).
- States: loading skeleton, empty state, error toast, permission-gated hide/disable.
- Validation: Zod schema in `src/lib/schema/yard-flow/bijak-listSchema.ts`.

## Files to create
- `src/app/[locale]/yard-flow/...` — thin route wrapper
- `src/components/pages/yard-flow/loading-pipeline/bijak-list/`
- `src/services/yard-flow/bijakService.ts`
- `src/hooks/yard-flow/useBijaksMutations.ts`
