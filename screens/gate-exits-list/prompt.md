# Gate Exits — implementation prompt
...
## Goal
Gate Exits screen in the **Loading Pipeline** cluster. Used by Depot Tallyman, Control Office, Warehouse Guard, Vessel Tallyman.

## Route & placement
- Route: `/yard-flow/loading/gate`
- Sidebar: Yard Flow (L1 rail) → Loading & Exit (L2 cluster) → route cluster → Gate Exits (L4)
- Breadcrumb: Yard Flow / Loading / Gate Exits
...
- Endpoints:
  ...
  |--------|------|---------|-------------|--------------|
| `GET` | `/gate-exits` | Gate Exits action | — | — |
- Auth: mutations require `actor` field. Permissions: gateexit.final.confirm.


## Data model (frontend types to add)
- `src/lib/types/yard-flow/response/gate-exits-list/get-gate-exits-list.dto.ts`
- `src/lib/types/yard-flow/request/gate-exits-list/create-gate-exits-list-request.dto.ts`
- Enums: `src/lib/enums/yard-flow/gatecontrol-status.enum.ts` — values: Confirmed, Rejected, Reversed

## UI spec
- Component pattern: **GenericTable**
### Columns
- **Bijak No** (`bijakNo`) — filter: text
- **Truck** (`plate`) — filter: text
- **Status** (`status`) — filter: text, status badge
- **Confirmed At** (`date`) — filter: text

- Toolbar actions mapped to endpoints listed above.
- Status badges use semantic tones (green=confirmed, amber=draft, red=rejected, blue=in-progress).
- States: loading skeleton, empty state, error toast, permission-gated hide/disable.
- Validation: Zod schema in `src/lib/schema/yard-flow/gate-exits-listSchema.ts`.

## Files to create
- `src/app/[locale]/yard-flow/...` — thin route wrapper
- `src/components/pages/yard-flow/loading-pipeline/gate-exits-list/`
- `src/services/yard-flow/gatecontrolService.ts`
- `src/hooks/yard-flow/useGateExitsMutations.ts`
